...
class Head_MDM(nn.Module):
    ...
    def forward(self, text, action, vertice, random, temporal_mask):
        ...
        recon_action, mu, log_var = self.cvae(action_flat, enc_text)
        recon_action = recon_action.view(bs, self.max_seq_len, self.d_action)
        macro_guide = self.emb_macro(recon_action)
        macro_guide = self.pe(macro_guide)

        noise = torch.randn_like(macro_guide)
        macro_guide = macro_guide + noise
        hidden_states = self.transformer_encoder(macro_guide)
        ...
        losses["recon"] = masked_l2(recon_action, action, temporal_mask).mean()
        losses["kl-div"] = kl_div_loss(mu, log_var)

        losses["full"] = losses["verts"] + (losses["recon"] * 10) + losses["kl-div"]
        ...
        assert bs == 1, f"For Autoregressive training, the batch_size must be 1 (is: [{bs}])"
        ...
        #macro_guide = macro_guide + noise
        # V8.5
        macro_guide = torch.cat((macro_guide, random.unsqueeze(1).repeat(1, frame_num, 1)), dim = -1)
        macro_guide = self.pe(macro_guide)
        hidden_states = self.transformer_encoder(macro_guide)
        ...
        for i in range(frame_num):
            if i == 0:
                ...
                vertice_emb = torch.zeros((1, 1, self.latent_dim)).to(self.device)
    ...
    def predict(self, text, random):
        ...
        enc_text = self._encode_text(text).type(torch.get_default_dtype())
        recon_action = self.cvae.sample(enc_text)
        recon_action = recon_action.view(bs, self.max_seq_len, self.d_action)
        macro_guide = self.emb_macro(recon_action)
        ...
        #macro_guide = macro_guide + noise
        # V8.5
        macro_guide = torch.cat((macro_guide, random.repeat(1, self.max_seq_len, 1)), dim = -1)
        macro_guide = self.pe(macro_guide)
        hidden_states = self.transformer_encoder(macro_guide)

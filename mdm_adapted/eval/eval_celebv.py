...
def evaluate(model, diffusion, dataloaders, args, region_ids, base_face):
    ...
    with open(args.log_file, "w") as f:
        ...
        for i in range(args.replication_times):
            ...
            if args.generate_samples:
                ...
                data = generate_samples(model, diffusion, gt_loader, out_path)
            else:
...
def generate_samples(model, diffusion, dataloader, location):
    paths = []
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            samples = sample_fn(
                model,
                (args.batch_size, model.njoints, model.nfeats, dataloader.dataset.opt.max_seq_len),
                clip_denoised = False,
...
if __name__ == "__main__":
    ...
    evaluate(model, diffusion, (gt_loader, eval_loader), args, region_ids, base_face)

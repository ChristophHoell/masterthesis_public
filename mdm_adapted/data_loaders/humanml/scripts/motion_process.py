...
def recover_root_rot_pos(data):
    ...
    # (B, seq_len)
    r_rot_ang[..., 1:] = rot_vel[..., :-1]          # Shift by 1
    r_rot_ang = torch.cumsum(r_rot_ang, dim=-1)     # Calculate Cumulative sum over the shifted vector
    ...
    r_pos[..., 1:, [0, 2]] = data[..., :-1, 1:3]                    # Shift by 1 (and take only Angular velocity of XZ plane)
...
def recover_from_ric(data, joints_num):
    ...
    # Get Quarternion Rotation and Rotation Position from data
    # Root Rotation and Root position
    r_rot_quat, r_pos = recover_root_rot_pos(data) #(1, 1, 196, 4) (1, 1, 196, 3)

    # Relative positions to root
    positions = data[..., 4:(joints_num - 1) * 3 + 4]           # (1, 1, 196, 63)
    positions = positions.view(positions.shape[:-1] + (-1, 3))  # (1, 1, 196, 21, 3)

    '''Add Y-axis rotation to local joints'''
    # Add Root rotation inversely to the positions
    ...
    # Add Root position as joint position

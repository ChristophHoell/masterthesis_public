...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        # WordVectorizer -> splits the text prompt into Location, Body, Object, Action and Description
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
        ...
        # Open Dataset Split file (train.txt etc) and parse elements
        with cs.open(split_file, 'r') as f:
            for line in f.readlines():
                id_list.append(line.strip())
        # id_list = id_list[:200]
        ...
        # opt.motion_dir: DEFAULT: new_joint_vecs

        # Iterate through the dataset
        for name in tqdm(id_list):
            try:
                # Filter motion sequence depending on the length (accept only valid ones)
                # motion shape: (seq_len, 263)
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
                if (len(motion)) < min_motion_len or (len(motion) >= 200):
                    continue
                text_data = []
                ...
                # Open Text description file
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:

                    # Parse through line elements (different descriptions)
                    for line in f.readlines():
                        text_dict = {}

                        # split line into (caption -> raw text, token -> processed text by Noun, etc)
                        line_split = line.strip().split('#')
                        caption = line_split[0]
                        tokens = line_split[1].split(' ')

                        # extract f and to tag
                        f_tag = float(line_split[2])
                        to_tag = float(line_split[3])
                        f_tag = 0.0 if np.isnan(f_tag) else f_tag
                        to_tag = 0.0 if np.isnan(to_tag) else to_tag
                        ...
                        # If t and to tags are 0, then use original text & motion
                        # else use motion only between f and to step
                        if f_tag == 0.0 and to_tag == 0.0:
                            flag = True
                            text_data.append(text_dict)
                        else:
        ...
        # Zip names and length list
        name_list, length_list = zip(*sorted(zip(new_name_list, length_list), key=lambda x: x[1]))

        # Store mean, std and other important stuff
        self.mean = mean
        self.std = std
        self.length_arr = np.array(length_list)
        self.data_dict = data_dict
    ...
    def reset_max_len(self, length):
        """
            Limits the max length of the dataset by creating an artificial pointer as start point
        """
    ...
    def __getitem__(self, item):
        # get absolute idx depending on pointer
        idx = self.pointer + item
        ...
        """
            data.keys:          motion, length, text
            data["motion"]:     (var, 263)
            data["length"]:     (var)
            data["text"]:       {caption, tokens}
        """

        # unpack dictionary
        motion, m_length, text_list = data['motion'], data['length'], data['text']

        # Randomly select a caption & unpack text dictionary
        text_data = random.choice(text_list)
        caption, tokens = text_data['caption'], text_data['tokens']

        # Pad / crop text if over / under max length
        if len(tokens) < self.opt.max_text_len:
            # pad with "unk"
            tokens = ['sos/OTHER'] + tokens + ['eos/OTHER']
            sent_len = len(tokens)
        ...
        # Obtain word embedding and positional encoding for the text tokens
        for token in tokens:
            word_emb, pos_oh = self.w_vectorizer[token]
            pos_one_hots.append(pos_oh[None, :])
            word_embeddings.append(word_emb[None, :])
        ...
        # Padd motion sequence to max length with 0
        ...
        # Output gets processed by tensors.collate_fn before given to network


        """
            word_embedding: (22, 300)
            pos_one_hots:   (22, 15)
            caption:        String
            sent_len:       Varying     -> caption length + sos/ and eos/ (Start / End of Sentence)
            motion:         (196, 263)
            m_length:       Varying
            tokens:         tokenized caption
        """

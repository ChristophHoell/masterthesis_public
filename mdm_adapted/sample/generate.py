...
def main():
    ...
    if is_using_data:
        ...
        collate_args = [{'inp': torch.zeros(n_frames), 'lengths': n_frames, "random": torch.randn(args.batch_size, data.dataset.opt.d_random)}] * args.num_samples
    ...
    for rep_i in range(args.num_repetitions):
        ...
        for i, s in enumerate(sample):
            ...
            motion = data.dataset.inv_normalize(motion.detach().cpu())
            all_motions.append(motion)
            text = model_kwargs["y"]["text"][i]
            all_text.append(text)

            if args.render_sample:
                name = text.replace(" ", "_").replace(".", "").replace(",", "")[:30] + f"_{rep_i}"
                renderer.render_vertices(motion, name, text)

        """
        all_motions.append(sample.detach().cpu())
    ...
    torch.save(data, os.path.join(out_path, "results.frame"))

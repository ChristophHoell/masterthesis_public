...
class T2MUnet(nn.Module):
    ...
    def __init__(self,         
        ...
        self.enc = nn.Linear(15069, self.input_feats)
        self.dec = nn.Linear(self.input_feats, 15069)

        #logger.critical(f"Parameters: latent {self.latent_dim}, inp_feats {self.input_feats}, dim_mults {self.dim_mults}, base_dim {self.base_dim}, cond {self.cond_mask_prob}")
        #for p in self.parameters():
        #    logger.critical(f"parameter: {p.name} - {p.numel()}")
        #logger.critical(f"Total params {(sum(p.numel() for p in self.parameters()) / 1000000.0):.2f}M")
    ...
    def forward(
        ...
        #logger.critical(f"X.shape into unet: {x.shape}")

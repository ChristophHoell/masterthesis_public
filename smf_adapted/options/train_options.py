...
class TrainOptions():
    ...
    def initialize(self):
        ...
        # Custom Changes:
        self.parser.add_argument("--data_dir", type = str, default = "./data/custom_reduced_90_frames/", help = "Path to the folder containing the dataset options file")
        ...
        self.parser.add_argument('--base_dim', type=int, default=128, help='Dimension of Unet base channel')
    ...
    def parse(self,accelerator):
        ...
        if self.opt.dataset_name == 't2m' or self.opt.dataset_name == 'humanml':
            ...
            self.opt.fps = 12.5
        elif self.opt.dataset_name == "celebv":
            self.opt.joints_num = 5023
            self.opt.dim_pose = 512
            self.opt.max_motion_length = 90
            self.opt.fps = 30
            self.opt.radius = 4
        else:
